...
func (p *PostServiceImpl) DeletePost(id string, Id string) error {
	obId, _ := primitive.ObjectIDFromHex(id)
	query := bson.M{"_id": obId}

	var post *models.DBPost
	err := p.postCollection.FindOne(p.ctx, query).Decode(&post)
	...
	if post.User != Id {
		return errors.New("not allowed to delete this document")
	}
